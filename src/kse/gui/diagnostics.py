...
wake-up helper and its test, and KSE in the menu and at login."""
...
from kse.cli.format import local, relative, span
from kse.config import Paths
from kse.doctor import WakeTest, run_wake_test, verdict_message
from kse.gui.client import DaemonLink
from kse.gui.icons import themed
from kse.gui.tasks import ask, inform, show_error, spawn
from kse.i18n import _
from kse.install.autostart import desktop_module
...
class DiagnosticsTab(QWidget):
    def __init__(
        ...
        *,
        ...
        daemon_box = QGroupBox(_("KSE daemon"))
        ...
        # Desktop
        self.menu_box = QCheckBox(_("Show KSE in the applications menu"))
    ...
    async def _start_daemon(self, service: ModuleType) -> None:
        ...
        text = _(
            "This installs KSE as a service of your user: it starts now and every time you log in."
        )
        if dry_run:
...
class HelperDialog(QDialog):
    ...
    def __init__(
        ...
        self.unattended.setToolTip(
            _("Lets KSE program wake-ups and power actions when nobody has a session open.")
        )
