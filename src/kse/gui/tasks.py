...
from kse.connection import ApiError, DaemonUnavailable
...
def show_error(error: Exception, parent: QWidget | None = None) -> None:
    ...
    box = QMessageBox(QMessageBox.Icon.Warning, "KSE", message(error), parent=parent)
...
def ask(
    parent: QWidget | None, text: str, on_yes: Callable[[], None], *, title: str = "KSE"
) -> QMessageBox:
...
def inform(parent: QWidget | None, text: str, *, warning: bool = False) -> QMessageBox:
    ...
    box = QMessageBox(icon, "KSE", text, parent=parent)
    box.setAttribute(Qt.WidgetAttribute.WA_DeleteOnClose)
    box.open()
    return box
