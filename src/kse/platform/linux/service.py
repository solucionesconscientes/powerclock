"""The daemon as a systemd user service (~/.config/systemd/user/kse.service)."""
...
def daemon_executable() -> Path:
    """kse-daemon next to the running interpreter (the venv of uv or pipx), else on PATH."""
    sibling = Path(sys.executable).parent / "kse-daemon"
    if sibling.exists():
        return sibling
    found = shutil.which("kse-daemon")
    if found is None:
        raise ServiceError("kse-daemon not found: is kse installed?")
    return Path(found)


def render_unit(executable: Path, *, dry_run: bool) -> str:
    environment = "Environment=KSE_DRY_RUN=1\n" if dry_run else ""
    return (
        "[Unit]\n"
        "Description=KShutdown Evolution daemon (power and task automation)\n"
        "After=dbus.service\n"
        "\n"
        "[Service]\n"
        "Type=simple\n"
