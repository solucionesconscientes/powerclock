...
MENU_FILE = "kse.desktop"
AUTOSTART_FILE = "kse-gui.desktop"
ICON = "kse"
...
def gui_executable() -> Path:
    """kse-gui as installed (pipx puts it on the PATH), else next to this Python."""
    found = shutil.which("kse-gui")
    return Path(found) if found else Path(sys.executable).parent / "kse-gui"
...
def set_menu(on: bool, env: Mapping[str, str] | None = None) -> list[str]:
    """Add KSE to the applications menu (with its icon), or take it out."""
    if not on:
        return _remove(menu_path(env), icon_path(env))
    icon = icon_path(env)
    icon.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(str(files("kse.gui") / "icons" / "kse.svg"), icon)
