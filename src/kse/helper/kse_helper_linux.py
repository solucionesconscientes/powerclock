#!/usr/bin/python3
"""kse-helper: the only part of kse that runs as root.

It programs, clears or reads the RTC wake-up alarm and does nothing else. It is installed
as /usr/local/libexec/kse-helper and called through pkexec (polkit action
org.kse.helper.wake):

    kse-helper wake-set <epoch>   alarm at a UTC Unix time (future, at most a year ahead)
    kse-helper wake-clear         remove the alarm
    kse-helper wake-get           print the alarm as a UTC Unix time, or "none"
...
def main(argv=None, *, now=None, rtcwake=None, wakealarm=WAKEALARM, adjtime=ADJTIME):
    ...
    try:
        if len(argv) == 2 and argv[0] == "wake-set":
            ...
            raise HelperError(
                "usage: kse-helper wake-set <epoch> | wake-clear | wake-get", EXIT_USAGE
            )
    except HelperError as exc:
        print(f"kse-helper: {exc}", file=sys.stderr)
        return exc.code
    return EXIT_OK
