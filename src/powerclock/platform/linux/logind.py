...
from powerclock.platform.base import NotSupported, PowerAction, PowerEvent, PowerEventCallback
from powerclock.platform.linux.dbus import Bus, get_property
...
class Logind:
    ...
    async def inhibitor(self, what: str, why: str, mode: str = "delay") -> AsyncIterator[None]:
        ...
        [fd] = await self._bus.call(
            LOGIND, MANAGER_PATH, MANAGER, "Inhibit", "ssss", [what, "powerclock", why, mode]
        )
        try:
            yield
        finally:
