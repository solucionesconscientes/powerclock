...
- **Aplicado en M15:** la regla leída como frase en lo alto del editor (`RuleSentence`: Cuándo · Solo si… · Esperar mientras… · Qué hará; cada pieza es un enlace a su pestaña; se refresca cada 400 ms desde los formularios y conserva la última frase si algo está a medio escribir) con `labels.describe_step`; horarios sin cron (`forms.ScheduleField` para `cron.expr`: todos los días, laborables, fines de semana, estos días, una vez al mes, cada N horas o minutos; una expresión que esas opciones no dirían exactamente igual se queda en «Avanzado: expresión cron», así nada se reescribe) y `labels.cron_words` para mostrarlos en palabras en todas partes; JSON en la pestaña «Avanzado (JSON)»; galería (`powerclock/gallery.py`, sin Qt: 17 plantillas en 6 casos de uso, todas validadas en un test; `<url>`, `<file>`… los rellena el usuario; la de horas baratas solo aparece con tarifa) que abre el editor con la plantilla.
...
│   ├── gallery.py       # reglas ya preparadas por caso de uso (M15)
...
│   └── gui/             # app.py (powerclock-gui) controller.py client.py (HttpApi, DaemonLink) tray.py window.py quick.py rules.py editor.py forms.py history.py diagnostics.py maintenance.py countdown.py setup.py (ventana de instalación) welcome.py summary.py widgets.py energy.py style.py cards.py gallery.py tasks.py single.py icons.py icons/*.svg
