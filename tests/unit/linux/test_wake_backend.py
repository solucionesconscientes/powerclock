"""Wake-up alarm on Linux: pkexec + kse-helper to write, sysfs to read (all simulated)."""
...
from kse.platform.linux.helper import packaged
...
def install_helper(root: Path, outdated: bool = False) -> None:
    helper = b"old helper" if outdated else packaged("kse_helper_linux.py").read_bytes()
    write(root, "usr/local/libexec/kse-helper", helper)
    write(root, "usr/share/polkit-1/actions/org.kse.helper.policy", "<policyconfig/>")
...
async def test_wake_set_and_clear_go_through_pkexec(tmp_path: Path) -> None:
    ...
    assert [argv for argv, _ in commands.ran] == [
        ["pkexec", "/usr/local/libexec/kse-helper", "wake-set", str(int(WHEN.timestamp()))],
        ["pkexec", "/usr/local/libexec/kse-helper", "wake-clear"],
    ]


async def test_wake_needs_the_helper(tmp_path: Path) -> None:
    commands = FakeCommands(available=["pkexec"])
    with pytest.raises(NotSupported, match="kse-helper is not installed") as info:
        await linux(tmp_path, commands).wake_set(WHEN)
    assert info.value.fix_hint == "kse helper install"
    assert commands.ran == []
...
async def test_doctor_without_helper(tmp_path: Path) -> None:
    ...
    assert report["wake.helper"] == (False, "not installed", "kse helper install")
...
async def test_doctor_with_helper(tmp_path: Path, code: int, supported: bool, text: str) -> None:
    ...
    assert pkcheck[:3] == ["pkcheck", "--action-id", "org.kse.helper.wake"]
...
async def test_doctor_notices_an_outdated_helper(tmp_path: Path) -> None:
    ...
    assert report["wake.helper"] == (
        False,
        "installed, but from another version of kse",
        "kse helper install",
    )
...
async def test_doctor_unattended_rule(tmp_path: Path) -> None:
    install_helper(tmp_path)
    write(tmp_path, "etc/polkit-1/rules.d/50-kse-unattended.rules", "// kse")
